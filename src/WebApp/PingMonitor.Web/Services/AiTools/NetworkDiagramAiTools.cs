...
internal sealed class ListNetworkDiagramsAiTool : NetworkDiagramAiToolBase
{
    ...
    public override async Task<AiToolExecutionResult> ExecuteAsync(AiToolCall call, CancellationToken cancellationToken)
    {
        ...
        var rows = await DbContext.NetworkDiagrams.AsNoTracking().OrderBy(x => x.Name).Take(call.Limits.MaxDiagramListResults + 1).Select(x => new { x.DiagramId, x.Name, x.Description, x.UpdatedAtUtc, nodeCount = x.Nodes.Count, linkCount = x.Links.Count, areaCount = x.Areas.Count }).ToArrayAsync(cancellationToken);
        return JsonResult(new { diagrams = rows.Take(call.Limits.MaxDiagramListResults).Select(x => new { diagramId = x.DiagramId, name = Truncate(x.Name, 160), description = Truncate(x.Description, 300), x.nodeCount, x.linkCount, x.areaCount, updatedAtUtc = x.UpdatedAtUtc }), featureEnabled = true, permissionFiltered = true, truncated = rows.Length > call.Limits.MaxDiagramListResults, returnedCount = Math.Min(rows.Length, call.Limits.MaxDiagramListResults), totalCount = rows.Length > call.Limits.MaxDiagramListResults ? (int?)null : rows.Length }, call.Limits.MaxDiagramToolResultCharacters);
    }
}
...
internal sealed class GetNetworkDiagramAiTool : NetworkDiagramAiToolBase
{
    ...
    public override async Task<AiToolExecutionResult> ExecuteAsync(AiToolCall call, CancellationToken cancellationToken)
    {
        ...
        var truncated = snapshot.Nodes.Count > call.Limits.MaxFullDiagramNodesReturned || snapshot.Links.Count > call.Limits.MaxFullDiagramLinksReturned || snapshot.Areas.Count > MaxFullAreas;
        return JsonResult(new { diagramId = snapshot.Diagram.DiagramId, name = snapshot.Diagram.Name, description = Truncate(snapshot.Diagram.Description, 500), generatedAtUtc = DateTimeOffset.UtcNow, source = "saved_network_diagram", isLiveLinkState = false, permissionFiltered = true, truncated, reason = truncated ? "Result exceeded AI tool size limit." : null, totalNodeCount = snapshot.Nodes.Count, totalLinkCount = snapshot.Links.Count, totalAreaCount = snapshot.Areas.Count, areas = snapshot.Areas.Take(MaxFullAreas).Select(a => new { areaId = a.AreaId, label = Truncate(a.Label, 160), notes = Truncate(a.Notes, 300), styleKey = a.StyleKey, containedNodes = snapshot.Nodes.Where(n => n.X >= a.X && n.Y >= a.Y && n.X <= a.X + a.Width && n.Y <= a.Y + a.Height).Select(n => n.NodeId).Take(40).ToArray() }), nodes = snapshot.Nodes.Take(call.Limits.MaxFullDiagramNodesReturned).Select(n => NodePayload(n, endpoints, snapshot.Areas)), links = snapshot.Links.Take(call.Limits.MaxFullDiagramLinksReturned).Select(l => LinkPayload(l, nodeById)), limitations = new[] { "This is saved diagram documentation.", "This is not live switch interface state.", "Diagram links do not create or modify monitoring dependencies." } }, call.Limits.MaxDiagramToolResultCharacters);
    }
